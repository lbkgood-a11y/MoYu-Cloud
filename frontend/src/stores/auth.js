...
export const useAuthStore = defineStore('auth', () => {
    ...
    const permissions = ref(JSON.parse(localStorage.getItem('moyu_permissions') || '[]'));
    const username = ref(localStorage.getItem('moyu_username') || '');
    const loggedIn = computed(() => Boolean(token.value));
    function setSession(accessToken, nextPermissions = [], user = '') {
    ...
    function clearSession() {
        ...
        localStorage.removeItem('moyu_permissions');
