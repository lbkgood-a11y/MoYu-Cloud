...
public class PermissionSchemaInitializer implements CommandLineRunner {
    ...
    private void initializeDefaultMenus() {
        jdbcTemplate.update("INSERT INTO sys_role (role_code, role_name, enabled) "
        ...
        insertMenu("用户查询", "system:user:read", "M");
        insertMenu("用户管理", "system:user:write", "B");
    ...
    private void insertMenu(String name, String permission, String type) {
        jdbcTemplate.update("INSERT INTO sys_menu (parent_id, menu_name, permission, menu_type, enabled) "
                + "SELECT 0, ?, ?, ?, 1 WHERE NOT EXISTS "
                + "(SELECT 1 FROM sys_menu WHERE permission = ?)", name, permission, type, permission);
    }
}
